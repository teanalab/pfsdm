...
shinyServer(function(input, output) {
  ...
  output$kwOutput <- renderPrint({ 
    filtered <- filter(merged, type == input$type)
    kruskal.test(filtered[[input$feature]], filtered$field)
  })
  output$kmcOutput <- renderPrint({ 
    filtered <- filter(merged, type == input$type)
    kruskalmc(filtered[[input$feature]], filtered$field)
  })
})