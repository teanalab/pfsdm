 ...
 */
object ImportanceFeatureValuesTable extends App {
  ...
  val importanceFeatureNames = parameters.getAsList("tableFeatures", classOf[String])
  ...
  val features: Map[String, ImportanceFeature] = importanceFeatureNames.map { f => f -> ImportanceFeature(f, retrieval) }.toMap

  val output = new PrintWriter(mainParameters.getString("output"))

  output.println(Seq("qid", "ngramtype", "gram", "featurename", "featurevalue").mkString("\t"))
  for ((qId, grams) <- uniBiGrams; gram <- grams) {
    val ngramtype = gram.length match {
      case 1 => "unigram";
      case 2 => "bigram"
    }
    for (featureName <- importanceFeatureNames) {
      try {
        val phi = features(featureName).getPhi(gram, qId)
        output.print(qId)
        output.print("\t")
