 ...
 */
public class FieldedFullDependenceTraversal extends MLMTraversal {
...
//    protected final int uwWidth;
    ...
    public FieldedFullDependenceTraversal(Retrieval retrieval) {
        super(retrieval);

        unigramDefault = globals.get("uniw", 0.8);
        orderedDefault = globals.get("odw", 0.1);
        unorderedDefault = globals.get("uww", 0.1);
        windowLimitDefault = (int) globals.get("windowLimit", 3);
...
//        uwWidth = (int) globals.get("sdm.uw.width", 8);
    ...
    public Node afterNode(Node original, Parameters qp) throws Exception {
        if (original.getOperator().equals("fieldedfdm")) {
        ...
        for (List<Node> set : nodePowerSet) {
            if ((windowLimit < 2) || (windowLimit >= set.size())) {
                if (set.size() >= 2) {
                    ...
                    double odWeight = computeWeight(set, np, qp, true);
                    BigramNodes bigramNodes = getBigramNodes(original, qp, set, uwSize);
                    ...
                    double uwWeight = computeWeight(set, np, qp, false);
                    if (uwWeight != 0) {
...
//        }
    ...
    protected BigramNodes getBigramNodes(Node original, Parameters qp, List<Node> seq, int uwWidth) throws Exception {
        NodeParameters np = original.getNodeParameters();
